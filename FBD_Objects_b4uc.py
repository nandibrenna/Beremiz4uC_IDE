...
BORDER_COL 	= wx.Colour('#2D2727')
BOX_COL 	= wx.Colour('#C4C5D4')
BOX_BORDER 	= 2
BOX_RADIUS 	= 3
...
# Draw a rectangle with rounded corners and a gray background
def DrawFBDBox(dc, x, y, w, h):
    dc.SetBrush(wx.Brush(BOX_COL, wx.BRUSHSTYLE_SOLID))
    dc.SetPen(wx.Pen(BORDER_COL, BOX_BORDER))
    dc.DrawRoundedRectangle(x, y, w, h, BOX_RADIUS)
...
#        return Graphic_Element.OnMotion(self, event, dc, scaling)
    ...
    def Draw(self, dc):
        ...
        DrawFBDBox(dc, self.Pos.x, self.Pos.y, self.Size[0] + 1, self.Size[1] + 1)
...
class FBD_Connector(Graphic_Element):
    ...
    def Draw(self, dc):
        ...
        text_pos = (self.Pos.x + (self.Size[0] - name_size[0]) // 2,
                    self.Pos.y + (self.Size[1] - name_size[1]) // 2)
        dc.DrawText(self.Name + 'Test', text_pos[0], text_pos[1])
        # Draw connector
        if self.Connector:
